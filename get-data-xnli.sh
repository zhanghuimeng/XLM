...
if [ ! -d $OUTPATH/XNLI-MT-1.0 ]; then
  if [ ! -f $OUTPATH/XNLI-MT-1.0.zip ]; then
    # 原来的下载地址挂掉了==
    wget --no-check-certificate -c https://www.nyu.edu/projects/bowman/xnli/XNLI-MT-1.0.zip -P $OUTPATH
  fi
...
if [ ! -d $OUTPATH/XNLI-1.0 ]; then
  if [ ! -f $OUTPATH/XNLI-1.0.zip ]; then
    # 原来的下载地址挂掉了==
    wget --no-check-certificate -c https://www.nyu.edu/projects/bowman/xnli/XNLI-1.0.zip -P $OUTPATH
  fi
