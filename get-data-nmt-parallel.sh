...
SRC_TOK=$SRC_RAW.tok
TGT_TOK=$TGT_RAW.tok
PARA_SRC_RAW=$PARA_PATH/$SRC/corpus.$SRC
PARA_TGT_RAW=$PARA_PATH/$TGT/corpus.$TGT
PARA_SRC_TOK=$PARA_SRC_RAW.tok
PARA_TGT_TOK=$PARA_TGT_RAW.tok
...
if [ "$SRC" == "de" -o "$TGT" == "de" ]; then
  ...
  cd $MONO_PATH/de
  if ! [[ -f "news.2007.de.shuffled.gz" ]]; then
...
cd "2017/"
wget -c http://data.statmt.org/wmt17/translation-task/preprocessed/de-en/corpus.tc.en.gz
wget -c http://data.statmt.org/wmt17/translation-task/preprocessed/de-en/corpus.tc.de.gz
cd "../2018"
wget -c http://data.statmt.org/wmt18/translation-task/preprocessed/de-en/corpus.gz
cd ..

# decompress parallel data
# 现在的问题是我不知道解压出来是个啥（现在知道了……）
# 以及是否需要shuffle
echo "Decompressing parallel data..."
if [ ! -f "2017/corpus.tc.en" ]; then
  echo "Decompressing 2017/corpus.tc.en.gz..."
  gunzip -k 2017/corpus.tc.en.gz
else
...
echo "$SRC parallel data concatenated in: $PARA_SRC_RAW"
echo "$TGT parallel data concatenated in: $PARA_TGT_RAW"

# check number of lines

# Tokenize data

if ! [[ -f "$PARA_SRC_TOK" ]]; then
  echo "Tokenize $SRC parallel data..."
  eval "cat $PARA_SRC_RAW | $SRC_PREPROCESSING > $PARA_SRC_TOK"
fi

if ! [[ -f "$PARA_TGT_TOK" ]]; then
  echo "Tokenize $TGT parallel data..."
  eval "cat $PARA_TGT_RAW | $TGT_PREPROCESSING > $PARA_TGT_TOK"
fi
echo "$SRC parallel data tokenized in: $PARA_SRC_TOK"
echo "$TGT parallel data tokenized in: $PARA_SRC_TOK"
